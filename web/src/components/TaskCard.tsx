...
const TaskCard = ({task:{ title, description, id, persona, completed, group}, parent, handleTaskDone, rerender } : {task: { title: string, description: string, id: number, persona: string, completed: boolean, group: number}, parent: string, handleTaskDone:(title: string)=>void, rerender:()=>void }) => {
  ...
  useEffect(() => {
    if(parent==='To-Do'){
      setClassName1('bg-gray-1 text-gray-900 border-2 border-gray-2');
    }else if(parent==='In Progress'){
      setClassName1('bg-blue-1 border-2 border-blue-1 text-white');
    }else{
      setClassName1('bg-[#52C41A]')
    }
  },[parent])

  const renderEditPage = useCallback(() => {
    if(showUpdateTask){
      return (
  ...
  },[handleEdit, showUpdateTask, title1])
    ...
    <div className='bg-white flex flex-col h-full'>
      <div className='flex justify-between items-center border-b-2 border-light-1 px-6 py-3'>
        ...
        { (parent==='To-Do' || parent==='In Progress') &&
            <button className={`text-white pr-3 pl-2 gap-1 py-[3px] font-normal flex justify-between ${className1}`}
